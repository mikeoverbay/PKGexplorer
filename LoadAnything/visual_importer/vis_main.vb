...
remove_more:
    ...
    Private Function PrettyPrint(XML As [String]) As [String]
        ...
        'XML = XML.Replace("<Texture>", vbCrLf + "<Texture>")
...
remove_more:
    ...
    Public Function openXml_stream(ByVal f As MemoryStream, ByVal PackedFileName_in As String) As Boolean
        ...
        If head = Packed_Section.Packed_Header Then
            ...
            TheXML_String = TheXML_String.Replace("map_", PackedFileName_in)
        ElseIf head = Binary_Header Then
        Else
            Dim data(f.Length) As Byte
            f.Position = 0
            f.Read(data, 0, f.Length)
            TheXML_String = Encoding.UTF8.GetString(data, 0, data.Length)
            TheXML_String = TheXML_String.Replace(vbCrLf, vbLf)
